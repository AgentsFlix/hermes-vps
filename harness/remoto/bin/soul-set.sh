...
docker exec -i --user hermes hermes sh -c "cat > /opt/data/$alvo.novo && mv /opt/data/$alvo.novo /opt/data/$alvo"
docker exec --user hermes hermes sh -c "wc -c < /opt/data/$alvo" | sed "s/^/$alvo: /; s/\$/ bytes/"
